...
float deg = -75; // Start at 15 degrees
...
int tbuffer = 300;
float chance = 25.; // percentage of time particles are 
...
void draw() 
{
  ...
  if (diagState) {
    //textMode(SCREEN);
    textAlign(LEFT);
    if (gameStatus==0) text("Kinect FR: " + (int)kinect.getDepthFPS(),10,36);
    text("Processing FR: " + (int)frameRate,10,66);
  ...
  if (gameStatus == 0 ) {
    ...
    float radius=0.;
    // find blobs
    blobs = opencv.blobs( 20, 100000, 10, false , 500);
    // draw blob results
    for( int i=0; i<blobs.length; i++ ) {
      ...
      for( int k=0; k<pBlobs.length; k++ ) {
        ...
        if (dist < 30.) {
          matched=true;
          vxin=constrain(-1.0*vScale*distx,-1.0*vMax,vMax);
          vyin=constrain(1.0*vScale*disty,-1.0*vMax,vMax);
...
class Ball {
  ...
  void gravitate() {
    for (int i = id + 1; i < min(nB,numBalls); i++) {
      ...
      float soft=(diameter+cloud[i].diameter)/3.;
      if (distance >soft) {
        ...
        force = G*mass*cloud[i].mass/(soft*soft);
        a1=(force/mass)*(dx/soft);
        a2=(force/mass)*(dy/soft);
      }
      ax+=a1;
      ay+=a2;
